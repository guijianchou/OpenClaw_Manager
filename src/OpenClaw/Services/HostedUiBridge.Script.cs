...
internal static class HostedUiBridgeScript
{
    ...
    private static readonly Lazy<string> ModelResolverScript = new(LoadModelResolverScript);
...
""";
    ...
    private static string LoadModelResolverScript()
    {
        var assembly = typeof(HostedUiBridgeScript).Assembly;
        using var stream = assembly.GetManifestResourceStream(ModelResolverResourceName)
            ?? throw new InvalidOperationException($"Missing embedded resource {ModelResolverResourceName}.");
